...
$adtSession = @{
    ...
    AppScriptDate = '2026-01-07'
    AppScriptAuthor = 'netIP - PMK'
    RequireAdmin = $true
...
function Install-ADTDeployment
{
    ...
    if ($installedversion.DisplayVersion -like "7.4.3.*") {
    Start-ADTProcess -FilePath hotfix.exe -ArgumentList '-q'
    } else {
    Start-ADTMsiProcess -Action 'Install' -FilePath 'FortiClientVPN7.4.3.8758.msi' -ArgumentList "/quiet /norestart"
    }
    ...
    ## Indlæs kundespecifikke værdier fra ekstern config (holdes uden for Git).
    ## Falder tilbage til skabelonen, hvis den lokale fil ikke findes.
    $customerSettingsPath = "$PSScriptRoot\Config\CustomerSettings.psd1"
    if (-not (Test-Path -LiteralPath $customerSettingsPath))
    {
        $customerSettingsPath = "$PSScriptRoot\Config\CustomerSettings.Example.psd1"
    }
    $customerSettings = Import-PowerShellDataFile -LiteralPath $customerSettingsPath

    $vpnName = $customerSettings.VpnName               #Navn på forbindelsen når man åbner appen
    $vpnDescription = $customerSettings.VpnDescription #Beskrivelse når man editerer forbindelsen i appen
    $vpnServer = $customerSettings.VpnServer           #Selve stien til SSLVPN
    ...
    Set-ADTRegistryKey $regPath -Name 'Installed' -Type 'String' -Value $appVersion
...
function Uninstall-ADTDeployment
{
    ...
    $fortiClientInstalled = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*" | Where-Object { $_.DisplayName -like "*FortiClient*" }
    if ($fortiClientInstalled) {
    msiexec /x $($fortiClientInstalled.PSChildName) /qn /norestart REBOOT=ReallySuppress
    }
